...
{
    public partial class frmFinishedGoods : Form
    {
        //Create instance of FinishedGoods_Mst table in database to perfrom CRUD operations.
        FG_MST FinishedGoods = new FG_MST();
        ...
        {
            common.GetUserAccessForCurrentScreen(this, "Finished Goods");
        ...
        private void frmFinishedGoods_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateForm())
                {
                    ...
                    FinishedGoods.FG_NAME = txtFGName.Text.Trim();
                    FinishedGoods.FG_TYPE = cmbFGType.Text;
                    FinishedGoods.DESCRIPTION = txtFGDescription.Text.Trim();
                    ...
                    using (AI_BTS_DENSOEntities1 db = new AI_BTS_DENSOEntities1())
                    {
                        ...
                        if (FinishedGoods.FG_MST_ID == 0)
                        {
                            db.FG_MST.Add(FinishedGoods);
                            isAddOpr = true;
                        }
                        else
                            db.Entry(FinishedGoods).State = System.Data.Entity.EntityState.Modified;
        ...
        private void dgvData_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvData.CurrentRow.Index != -1)
                {
                    ...
                    FinishedGoods.FG_MST_ID = Convert.ToInt32(dgvData.CurrentRow.Cells["FG_ID"].Value);

                    using (AI_BTS_DENSOEntities1 db = new AI_BTS_DENSOEntities1())
                    {
                        FinishedGoods = db.FG_MST.Where(x => x.FG_MST_ID == FinishedGoods.FG_MST_ID).FirstOrDefault();
                        txtFGName.Text = FinishedGoods.FG_NAME;
                        txtFGDescription.Text = FinishedGoods.DESCRIPTION;
                        cmbFGType.Text = FinishedGoods.FG_TYPE.ToString();
                    }
                }
            }
            catch(Exception ex)
            ...
            try
            {
                ...
                else
                {
                    if (common.ConfirmToDelete())
                    {
                        using (AI_BTS_DENSOEntities1 db = new AI_BTS_DENSOEntities1())
                        {
                            if (db.Entry(FinishedGoods).State == System.Data.Entity.EntityState.Detached)
                                db.FG_MST.Attach(FinishedGoods);
                            db.FG_MST.Remove(FinishedGoods);
                            db.SaveChanges();
                            RefreshDataGridView();
        ...
        private bool ValidateForm()
        {
            ...
            try
            {
                if (txtFGName.Text.Trim() == "" || txtFGName.Text == null)
                    ClsMessage.ShowError("Please enter the Finished Goods name.");
                if (txtFGDescription.Text.Trim() == "" || txtFGDescription.Text == null)
                    ClsMessage.ShowError("Please enter the Finished Goods description.");
                else if (cmbFGType.Text.Trim() == "")
                    ClsMessage.ShowError("Please select the Finished Goods type.");
                else
                    IsFormValid = true;
            }
            catch(Exception ex)
        ...
        /// <summary>
        /// Refreshes the data grid view with FinishedGoods table data from database.
        /// </summary>
        private void RefreshDataGridView()
        {
            try
            {
                ...
                using (AI_BTS_DENSOEntities1 db = new AI_BTS_DENSOEntities1())
                {
                    dgvData.DataSource = db.FG_MST.ToList<FG_MST>();
                }
            }
            catch(Exception ex)
            {
        ...
        {
            common.SearchData(ref dgvData,"FG_MST", common.lstrCurrentColumnName, txtSearch.Text.Trim());
        }
